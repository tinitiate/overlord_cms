...
class CodeFileParser:
    ...
    def MultiLineBlockBuilder(self, p_block_name, p_data, p_block_state):    
        ...
        if p_block_name != ">>CODE-ALL-OS<<":
            # print(p_block_name)
            # l_data       = p_data.replace(" ","&nbsp;")
            l_data       = re.sub('^' + self.g_comment_char, '', p_data)
        else:
            l_data       = p_data
        ...
        if (p_block_state > 1):
            # print(p_block_name,p_block_state,l_data)
            
            l_non_pre_blocks = [">>PROS<<", ">>CONS<<", ">>CODE-NOTES<<", ">>NOTES<<"]


            # if p_block_name != ">>CODE-ALL-OS<<":
            #     self.g_multi_block_stage  = self.g_multi_block_stage + l_data
            if p_block_name in l_non_pre_blocks:
                self.g_multi_block_stage  = self.g_multi_block_stage + l_data + "<br>"     
            else:
                self.g_multi_block_stage  = self.g_multi_block_stage + l_data

            # self.g_multi_block_stage  = self.g_multi_block_stage + l_data
            
            # g_render_page = g_render_page + "<br>" + l_data;
        ...
        # End Block processing
            ...
            self.g_page_block_data_list.append([p_block_name,self.g_multi_block_stage])
    ...
    def FileParser(self, p_file_name_path):
        ...
        l_single_line_block     = ('>>TITLE - ','>>BREADCRUMB - '
                                    ,'>>HEADLINE - ','>>AUTHOR - '
                                    ,'>>DATEPUBLISHED - ','>>FILE-NAME - ','>>TAGS - '
                                    ,'>>DEPENDANT-FILES - ','>>CODE-TITLE - ','>>IMAGE - ');
        ...
        while True:
            ...
            if (l_curr_block_state == 1):
                ...
                # Reset Temp Multi Block Data  
                self.g_multi_block_stage = ""  
